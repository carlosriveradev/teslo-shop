...
const ProductAdminPage:FC<Props> = ({ product }) => {
    ...
    const onNewTag = () => {
        ...
        if(currentTags.includes(newTag) || newTag.length < 1){
            return
        }
    ...
    const onFilesSeletecd = async({target}: ChangeEvent<HTMLInputElement>) => {
    ...
    const onDeleteImage = (image: string) => {
        setValue('images',
        getValues('images').filter(img=>img!==image), {shouldValidate: true})
    }
